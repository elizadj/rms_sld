...
- [ ] **2026-05-13 — Export SLD per project as PDF.** Add a PDF export option, ideally under the Export menu (not Print) to avoid the browser's HTML page headers/footers/URL artefacts. One PDF per project, all pages of all selected SLDs included. Approach options: (a) use the existing print path but inject a hidden iframe + window.print() with `@page` styles only — still browser-print under the hood; (b) use a JS library like jsPDF or html2pdf for proper offline-rendered PDFs; (c) use the browser's built-in `print-to-PDF` flow with custom A3 landscape styling (closest to current Print Page / Print All, just no page header). Probably (b) for the cleanest output — adds a CDN dependency.
